...
const nazePath = fileURLToPath(new URL('../Hc.js', import.meta.url));
...
const reloadHandler = async () => {
	try {
		nazeHandler = (await import(`../Hc.js?update=${Date.now()}`)).default;
	} catch (err) {
...
async function LoadDataBase(naze, m) {
	try {
		...
		const defaultSetBot = {
			...
			privateonly: true,
			didyoumean: true,
			author: global.author || 'Heart candy',
			authorPrefix: '',
			autobackup: false,
			botname: global.botname || 'Hc-bot',
			packname: global.packname || 'Bot Heart candy',
